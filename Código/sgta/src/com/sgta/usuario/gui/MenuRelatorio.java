...
public class MenuRelatorio extends JFrame {
	...
	public MenuRelatorio() {
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (cpf.getText().equals("   .   .   -  ")) {
					Toast.makeText(frame, "Informe o cpf do Aluno.", 2000,
							Style.ERROR).display();
				} else {
					Pessoa aluno;
					try {
						...
						if (aluno.getNome() == null) {
							...
							items = (String[]) datas.toArray(new String[datas
									.size()]);
						}
		...
		/*
		 * List datas = new ArrayList<String>(); List<Medidas> listMedidas =
		 * business.retornaMedidasByUsuario(aluno.getUsuario().getId());
		 * datas.add(""); if (!listMedidas.isEmpty()) { for (Medidas p :
		 * listMedidas) { datas.add(p.getData().toString()); } } String[] items
		 * = (String[]) datas.toArray(new String[datas.size()]);
		 */

		JComboBox comboBoxData = new JComboBox(items);
